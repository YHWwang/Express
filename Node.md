# Node
Node.js是一个基于事件驱动、非阻塞I/O模型的服务器端JavaScript运行环境。它可以让JavaScript脱离浏览器运行在服务器端，支持高并发、低延迟、分布式计算等特性。同时，由于Node.js使用了Google V8引擎进行编译执行，其性能也相当出色。

# Libuv
libuv 是一个用 C 编写的支持多平台的异步 I/O 库，主要解决 I/O 操作容易引起阻塞的问题
1. Node.js 的核心依赖 libuv的结构可以分两部分，一部分是网络 I/O，底层实现会根据不同操作系统依赖不同的系统 API
2. 另一部分是文件 I/O、DNS、用户代码，这一部分采用线程池来处理。
libuv 处理异步操作的核心机制是:事件轮询，事件轮询分成若干步骤，大致操作是遍历并执行队列中的回调函数。
