...
# JQuery链式调用原理 ($("div").width("100px").height("100px");)
    let Fun={
      fn1:function(){
          console.log("fn1");
          return this;
...
Fun.fn1().fn2();这样子便可以实现链式了，原因是在每一个方法后面都加了一个return this。
1.在一个对象里面，this指向的是对象本身，当我们调用方法的时候，这些方法都是在对象内部调用的，所以加this才可以访问到这些方法。
...
# javascript的作用域的理解
回答：作用域就是一套规则，用于确定在何处以及如何查找变量（标识符）的规则
...
实现闭包常见的两种方式：1.函数作为参数被传递，2.函数作为返回值被返回
特性：
1、闭包可以访问当前函数以外的变量
